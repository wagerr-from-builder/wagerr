# TOR SUPPORT IN WAGERR CORE

It is possible to run Wagerr Core as a Tor hidden service, and connect to such services.
...
## 1. Run Wagerr Core behind a Tor proxy

The first step is running Wagerr Core behind a Tor proxy. This will already make all
...
port 9050 and only allows .onion nodes to connect:
	...
	./wagerrd -onion=127.0.0.1:9050 -onlynet=tor -listen=0 -addnode=ssapp53tmftyjmjb.onion
...
## 2. Run a Wagerr Core hidden server
...
your wagerrd's P2P listen port (9999 by default).
	...
	-externalip=X   You can tell Wagerr Core about its publicly reachable address using
	                ...
	                /var/lib/tor/wagerrcore-service/hostname. Onion addresses are given
...
Wagerr Core has been updated to make use of this.

This means that if Tor is running (and proper authentication has been configured),
Wagerr Core automatically creates a hidden service to listen on. This will positively 
affect the number of available .onion nodes.

This new feature is enabled by default if Wagerr Core is listening (`-listen`), and
requires a Tor connection to work. It can be explicitly disabled with `-listenonion=0`
and, if not disabled, configured using the `-torcontrol` and `-torpassword` settings.
To show verbose debugging information, pass `-debug=tor`.

Connecting to Tor's control socket API requires one of two authentication methods to be 
configured. For cookie authentication the user running wagerrd must have write access 
...
- Do not add anything but Wagerr Core ports to the hidden service created in section 2.
