...
- [Development guidelines](#development-guidelines)
    - [General Wagerr Core](#general-wagerr-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Wagerr Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/wagerr/wagerr/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_wagerr
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_wagerr --log_level=test_suite
$ valgrind -v --leak-check=full src/wagerrd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_wagerr.coverage/index.html`.
```

**Sanitizers**

Wagerr Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Wagerr Core, where everyone uses
...
pay attention to for reviewers of Wagerr Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Wagerr Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Wagerr Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `wagerr-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `wagerr-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Wagerr Core, the wallet was always
