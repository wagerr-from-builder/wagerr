...
DoCodeSign ".app" "output/Wagerr-Qt.app"
...
echo "Generating ZIP archive Wagerr-Qt.zip..."
ditto -c -k --rsrc --keepParent "output/Wagerr-Qt.app" "output/Wagerr-Qt.zip"

# Submit for notarization
echo "Submitting Wagerr-Qt for notarization..."
RESULT=$(xcrun altool --notarize-app --type osx \
  --file "output/Wagerr-Qt.zip" \
  --primary-bundle-id org.wagerr.Wagerr-Qt \
  --username $APPLE_ID_USER \
  --password @env:APPLE_ID_PASSWORD \
  --output-format xml)

if [ $? -ne 0 ]; then
  echo "Submitting Wagerr-Qt failed:"
  echo "$RESULT"
  exit 1
fi
...
do
  ...
  if [ "$STATUS" = "success" ]; then
    echo "Notarization of Wagerr-Qt succeeded!"
    break
  elif [ "$STATUS" = "in progress" ]; then
    echo "Notarization in progress..."
    sleep 20
  else
    echo "Notarization of Wagerr-Qt failed:"
    echo "$RESULT"
    exit 1
  fi
done
...
xcrun stapler staple "output/Wagerr-Qt.app"

# rm zip
rm "output/Wagerr-Qt.zip"

echo "Creating .DMG"
hdiutil create -fs HFS+ -volname Wagerr-Qt -srcfolder output/Wagerr-Qt.app output/wagerr-$VERSION-osx.dmg
DoCodeSign ".DMG" "output/wagerr-$VERSION-osx.dmg"