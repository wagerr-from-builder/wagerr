...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. wagerr_de.qm (shortcut "de" needs to be defined in wagerr.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. wagerr_de_DE.qm (shortcut "de_DE" needs to be defined in wagerr.qrc)
...
static const char* qt_argv = "wagerr-qt";
...
{
    QMessageBox::critical(nullptr, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Wagerr Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"WAGERR Core - \"").translated, ArgsManager::ALLOW_ANY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    /// 6. Determine availability of data directory and parse wagerr.conf
