...
static void UpdateMempoolForReorg(DisconnectedBlockTransactions& disconnectpool, bool fAddToMempool) EXCLUSIVE_LOCKS_REQUIRED(cs_main, ::mempool.cs)
{
    ...
    while (it != disconnectpool.queuedTx.get<insertion_order>().rend()) {
        ...
        if (!fAddToMempool || (*it)->IsGenerated() ||
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue, bool isZWGRStake)
{
    int64_t ret = 0;

    if (nHeight >= Params().GetConsensus().nPosStartHeight) {
        if (isZWGRStake){
            //When zWAGERR is staked, masternode gets less WAGERR
        ...
        for (const auto& tx : block.vtx) {
            ...
            while (llmq::CInstantSendLockPtr conflictLock = m_isman->GetConflictingLock(*tx)) {
                if (m_clhandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                    LogPrint(BCLog::ALL, "ConnectBlock(WAGERR): chain-locked transaction %s overrides islock %s\n",
                    ...
                    return state.Invalid(ValidationInvalidReason::TX_CONFLICT_LOCK, error("ConnectBlock(WAGERR): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()), REJECT_INVALID, "conflict-tx-lock");
                }
            }
        }
    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (block.vtx.empty() || !block.vtx[1]->IsCoinStake())
            return  error("CheckBlock() : second tx is not coinstake");
...
static bool IsValidBlockTimeStamp(const int64_t nTime, const int nHeight, const Consensus::Params& params)
{
    ...
    return (nTime % params.nTimeSlotLength) == 0;
}
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams))
        return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-diffbits", strprintf("incorrect proof of work at %d", nHeight));
...
bool TestBlockValidity(CValidationState& state, llmq::CChainLocksHandler& clhandler, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!::ChainstateActive().ConnectBlock(block, state, &indexDummy, viewNew, bettingsViewNew, chainparams, true))
        return false;
