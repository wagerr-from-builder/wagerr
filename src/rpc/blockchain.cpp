...
UniValue getblockchaininfo(const JSONRPCRequest& request)
{
    ...
    for (int pos = Consensus::DEPLOYMENT_DIP0020; pos != Consensus::MAX_VERSION_BITS_DEPLOYMENTS; ++pos) {
...
UniValue scantokens(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "    \"scriptPubKey\" : \"script\",  (string) the script key\n"
            "    \"WAGERR_amount\" : x.xxx,       (numeric) The total amount in BYZ of the unspent output\n"
    ...
    if (request.params[0].get_str() == "status") {
        ...
        for (const auto& it : coins) {
            ...
            unspent.pushKV("scriptPubKey", HexStr(txo.scriptPubKey.begin(), txo.scriptPubKey.end()));
            unspent.pushKV("WAGERR_Amount", ValueFromAmount(txo.nValue));
