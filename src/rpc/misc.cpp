...
static UniValue debug(const JSONRPCRequest& request)
{
    RPCHelpMan{"debug",
        ...
        " - \"wagerr\" activates all Wagerr-specific categories at once;\n"
        ...
        RPCExamples {
            HelpExampleCli("debug", "wagerr")
    + HelpExampleRpc("debug", "wagerr+net")
    }}.Check(request);
...
static UniValue validateaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"validateaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The wagerr address to validate"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
...
static UniValue verifymessage(const JSONRPCRequest& request)
{
    RPCHelpMan{"verifymessage",
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The wagerr address to use for the signature."},
...
static UniValue getaddressbalance(const JSONRPCRequest& request)
{
    ...
    for (std::vector<std::pair<CAddressIndexKey, CAmount> >::const_iterator it=addressIndex.begin(); it!=addressIndex.end(); it++) {
        ...
        if (it->first.txindex == 0 && nHeight - it->first.blockHeight < Params().GetConsensus().COINBASE_MATURITY(it->first.blockHeight)) {
    ...
    if (request.fHelp)
        throw std::runtime_error(
            RPCHelpMan{"echo|echojson ...",
                ...
                "wagerr-cli and the GUI. There is no server-side difference.",
                {},
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Wagerr features */
    { "wagerr",               "mnsync",                 &mnsync,                 {} },
    { "wagerr",               "spork",                  &spork,                  {"command"} },
    { "wagerr",               "sporkupdate",            &sporkupdate,            {"name","value"} },
