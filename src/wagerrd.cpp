 ...
 *
 * This is the developer documentation of the reference client for an experimental new digital currency called Wagerr (https://www.wagerr.com/),
...
bool AppInit(int argc, char* argv[])
{
    ...
    if (gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||  gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version"))
    {
        ...
        {
            strUsage += "\nUsage:\n"
                  "  wagerrd [options]                     " + strprintf("Start %s Daemon", PACKAGE_NAME) + "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr, "Error: Command line contains unexpected token '%s', see wagerrd -h for a list of options.\n", argv[i]);
                return false;
            }
        }

        // -server defaults to true for wagerrd but not for the GUI so do this here
...
#endif
            fprintf(stdout, "Wagerr Core server starting\n");
...
int main(int argc, char* argv[])
{
    ...
    // Connect wagerrd signal handlers
    noui_connect();
